...
preprocess_diagnosis_sekunder <- function(file_path) {
  ...
  # Prioritize CKD (N18) if multiple diseases exist for a case_id
  filtered_data <- filtered_data %>%
    group_by(case_id) %>%
    arrange(desc(diagnosis_code == "N18"), .by_group = TRUE) %>%
    slice_head(n = 1) %>%
    ungroup()
  ...
  print("Data after prioritizing CKD (N18) and removing duplicates:")
...
write_csv(combined_diagnosis_sekunder, "combined_diagnosis_sekunder_with_timestamps.csv")
